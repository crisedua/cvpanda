...
export function createComponentLogger(componentName: string) {
  return {
    log: (message: string, data?: any) => {
      ...
      const logMessage = `[${timestamp}] [${componentName}] ${message} ${dataString}`;
      console.log(logMessage);
      return logMessage;
    },
    error: (message: string, error?: any) => {
      const timestamp = new Date().toISOString();
      const errorString = error ? (error.toString ? error.toString() : JSON.stringify(error)) : '';
      const logMessage = `[${timestamp}] [${componentName}] ERROR: ${message} ${errorString}`;
    ...
    warn: (message: string, data?: any) => {
      const timestamp = new Date().toISOString();
      const dataString = data ? JSON.stringify(data) : '';
      const logMessage = `[${timestamp}] [${componentName}] WARNING: ${message} ${dataString}`;
