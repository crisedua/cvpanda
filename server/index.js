...
app.post('/api/analyze-skill-gaps', async (req, res) => {
  try {
    ...
    const analysisPromise = (async () => {
      ...
      // Create a more concise version of the CV for analysis
      const conciseCvData = {
        skills: cvContent.skills || [],
        experience: (cvContent.work_experience || []).map(job => ({
          title: job.title,
      ...
      // Prepare prompt for OpenAI - more concise prompt
      ...
      const userPrompt = `
      # CV Information
      ${JSON.stringify(conciseCvData, null, 2)}
      ...
      // Call OpenAI with a faster model
      ...
      // Add actionable plan structure even if OpenAI didn't provide it
      if (!result.actionablePlan) {
        result.actionablePlan = {
          shortTerm: [],
          mediumTerm: [],
          longTerm: []
        };
        
        // Generate simple action items from recommendations
        if (result.recommendations && result.recommendations.length > 0) {
          result.recommendations.forEach(rec => {
            const actionItem = {
              action: rec.title,
              timeframe: rec.timeToAcquire || "1-3 months",
              resources: [{
                name: rec.title,
                type: rec.type || "course",
                link: rec.link || "",
                description: rec.description
              }]
            };
            
            if (rec.timeToAcquire && rec.timeToAcquire.includes("week")) {
              result.actionablePlan.shortTerm.push(actionItem);
            } else if (rec.timeToAcquire && rec.timeToAcquire.includes("month") && parseInt(rec.timeToAcquire) > 3) {
              result.actionablePlan.mediumTerm.push(actionItem);
            } else {
              result.actionablePlan.longTerm.push(actionItem);
            }
          });
        }
      }
      
      // Add industry insights if missing
      if (!result.industryInsights) {
        result.industryInsights = "Based on the job description, this role requires a combination of technical and soft skills. Focus on building practical experience with the missing skills identified above.";
      }
